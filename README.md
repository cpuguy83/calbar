...
The GTK popup supports user CSS overrides.

- Default override path: `~/.config/calbar/style.css`
- Optional config override: `ui.css_file`
- Full selector reference, examples, and compositor notes: `docs/styling.md`
...
3. If the broker is unavailable, CalBar opens the browser using an Outlook public client that supports localhost loopback redirects and reuses the MSAL token cache afterward
